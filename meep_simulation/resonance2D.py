...
w = 0.547  # width of unit  (for bigger gap not centered at 1.545um wavelength: 0.4 ~ 0.41)  
           # 0.533 for gap to centered at 1.545um wavelength

a = 0.544  # lattice constant 
r = 0.1643  # radius of holes  (biggest at r=(a*n_silicon)/(n_air+n_silicon))

A = 0.8312    # defect factor of a
B = 0.48    # defec factor of r
N1=4   #number of holes in the mirror
N2=8   #number of holes in defect (9holes=8spacing)
...
for i in range(N1):
    geometry.append(mp.Cylinder(r, center=mp.Vector3(A*a + d(N2) + a/2 + i*a), material=mp.air))
    geometry.append(mp.Cylinder(r, center=mp.Vector3(-(A*a + d(N2) + a/2 + i*a)), material=mp.air))
for i in range(N2+1):
    geometry.append(mp.Cylinder((B*r+r*(1-B)*(i*i)/(N2*N2)), center=mp.Vector3(A*a + d(i)-(A*a+(1-A)*a*(i**2)/(N2**2))/2), material=mp.air))
    geometry.append(mp.Cylinder((B*r+r*(1-B)*(i*i)/(N2*N2)), center=mp.Vector3(-(A*a + d(i)-(A*a+(1-A)*a*(i**2)/(N2**2))/2)), material=mp.air))
...
sim.run(mp.to_appended("res",mp.at_every(1/fcen/20, mp.output_efield_x),mp.at_end(mp.output_epsilon)), until=1/fcen) 
