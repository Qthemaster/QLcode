...
pad = 0.5    # padding between pml layer and last hole
...
s = mp.Source(
    ...
    component=mp.Ey,
    center=mp.Vector3(-0.5*sx+dpml,0,0),size=mp.Vector3(0,w,t)
    )
...
# reflected flux
refl_fr = mp.FluxRegion(center=mp.Vector3(-0.5*sx+dpml+0.1,0,0), size=mp.Vector3(0,2*w,2*t))
refl = sim.add_flux(fcen, df, nfreq, refl_fr)

# transmitted flux
tran_fr = mp.FluxRegion(center=mp.Vector3(0.5*sx-dpml,0,0), size=mp.Vector3(0,2*w,2*t))
tran = sim.add_flux(fcen, df, nfreq, tran_fr)



pt = mp.Vector3(0.5*sx-dpml-0.2, 0, 0)
...
cavity_refl_flux = mp.get_fluxes(refl)
cavity_tran_flux = mp.get_fluxes(tran)
...
for i in range(nfreq):
    ...
    Rs = np.append(Rs,-cavity_refl_flux[i]/straight_tran_flux[i])
    Ts = np.append(Ts,cavity_tran_flux[i]/straight_tran_flux[i])
...
# sim.run(mp.after_sources(mp.Harminv(mp.Ey, mp.Vector3(0, 0, 0), fcen, df)),
